  ...
  #endif
{
  ...
  requestTemperaturesTimestamp = 0;
}
...
void DallasTemperature::requestTemperaturesByAddress(uint8_t* deviceAddress)
{
  ...
  requestTemperaturesTimestamp = millis();
  Serial.print("requestTemperaturesTimestamp = ");
  Serial.println(requestTemperaturesTimestamp);
}
...
float DallasTemperature::getTempC(uint8_t* deviceAddress)
{
  ...
  unsigned long diffTime = millis() - requestTemperaturesTimestamp;
  int delayTime;
  
  switch (conversionDelay)
  {
    case TEMP_9_BIT:
      delayTime = 94 - diffTime;
      break;
    case TEMP_10_BIT:
      delayTime = 188 - diffTime;
      break;
    case TEMP_11_BIT:
      delayTime = 375 - diffTime;
      break;
    case TEMP_12_BIT:
    default:
      delayTime = 750 - diffTime;
      break;
  }
  
  if (delayTime > 0) {
    Serial.print("delaying for: ");Serial.println(delayTime);
    delay(delayTime);
  }
